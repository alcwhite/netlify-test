...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    try {
      ...
      let isUserValid = await oauth.getIsUserValid(token, user);
      cookieHeaders.push(getCookie("isUserValid", isUserValid, oauth.config.sessionExpiration));
      cookieHeaders.push(getCookie("user", user, oauth.config.sessionExpiration));
    } catch(e) {
      // Do nothing
      console.log( e );
    }
